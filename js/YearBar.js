...
function YearBar()
{
	...
	const scale_param = [
		{ // original
			year_labels: [-4000, -3000, -2000, -1000, 0,476, 1000, 1453, 1789, 2000],
...
function draw_scale() {
    ...
    // Helper function to draw colored segments
    function drawSegment(color, yearStart, yearEnd) {
        ctx.fillStyle = color;
        const xStart = x_from_year(yearStart);
        const xEnd = x_from_year(yearEnd);
        ctx.fillRect(xStart, 0, xEnd - xStart, _SIZE);
    }

    // Draw background for each segment
    drawSegment('#8B4513', -4300, -3000); // Brown
    drawSegment('#F4A460', -3000, 476);  // Sand color
    drawSegment('#FFA500', 476, 1453);  // Orange
    drawSegment('#90EE90', 1453, 1789); // Light green
    drawSegment('#00008B', 1789, 2100); // Dark blue

    // Overlay a vertical gradient
    const gradient = ctx.createLinearGradient(0, 0, 0, _SIZE);
    gradient.addColorStop(0, 'rgba(0, 0, 0, 0.8)'); // Top: Dark, 80% opacity
    gradient.addColorStop(0.5, 'rgba(0, 0, 0, 0)'); // Middle: Transparent
    gradient.addColorStop(1, 'rgba(0, 0, 0, 0.8)'); // Bottom: Dark, 80% opacity
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, width, _SIZE);

    // Draw year labels (foreground elements)
    ctx.fillStyle = 'black';
